...
port_INLINE void activity_ti1ORri1() {
   cellType_t  cellType;
   open_addr_t cell_neighbor,neighbor;
   uint8_t     i;
   sync_IE_ht  sync_IE;
...
/**
@brief Here, the frequency at which the packet is sent is calculated, and the packet is locally stored before being sent.

@lkn{Samu} In this function, we store the calculated tx frequency in the fourth element of the payload of our application. Furthermore, we set the tx frequency to the #SYNCHRONIZING_CHANNEL if we need to send an EB beacon.
...
/**
@brief This is an activity that indicated an error in transmission of data.

@lkn{Samu} In this function, we store the updated number of transmission attemps in the payload of our application.
*/
port_INLINE void activity_tie5() {
   // indicate transmit failed to schedule to keep stats
   schedule_indicateTx(&ieee154e_vars.asn,FALSE);
	 ///@internal [LKN-reTX-counter]
   ...
   measurements_changeHopReTxCnt(ieee154e_vars.dataToSend, ieee154e_vars.dataToSend->l4_length, ieee154e_vars.dataToSend->l2_retriesLeft);*/
   //ieee154e_vars.dataToSend->l4_payload[ieee154e_vars.dataToSend->l4_length-4] = ieee154e_vars.dataToSend->l2_retriesLeft;
...
/**
@brief This indicated the end of the receiving data packet. Here starts the parsing of the packet's data bytes.

@lkn{Samu} From this function, we store the value of RSSI in the payload of our application.
*/
...
port_INLINE void activity_ri6() {
   ...
   #if 0
   openserial_printError(COMPONENT_IEEE802154E,ERR_MAXRXACKPREPARE_OVERFLOWS,
                          (errorparameter_t)TsSlotDuration,
                          (errorparameter_t)6);
   #endif
...
port_INLINE void ieee154e_syncAsnOffset(void) {
   ...
   // determine the current slotOffset
   asnOffset = ieee154e_vars.asn.byte4;
   asnOffset = asnOffset % availableChannels;
   asnOffset = asnOffset << 16;
   asnOffset = asnOffset + ieee154e_vars.asn.bytes2and3;
   asnOffset = asnOffset % availableChannels;
   asnOffset = asnOffset << 16;
...
@lkn{Samu} Major changes to the function. We enable the calculations of the next frequency hop by checking the value of @ref HOPPING, otherwise we always choose the @ref SYNCHRONIZING_CHANNEL
